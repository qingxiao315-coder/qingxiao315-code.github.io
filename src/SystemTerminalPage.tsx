...
interface SystemTerminalPageProps {
  onNavigate: (view: string) => void;
  playSound: (type: 'click' | 'hover' | 'success' | 'error' | 'boot' | 'login' | 'button') => void;
}

const SystemTerminalPage: React.FC<SystemTerminalPageProps> = ({ onNavigate, playSound }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setSystemStats(prev => ({
        ...prev,
        memory: (parseFloat(prev.memory as unknown as string) + (Math.random() - 0.5) * 2).toFixed(1),
        cpu: (parseFloat(prev.cpu as unknown as string) + (Math.random() - 0.5) * 3).toFixed(1),
        bandwidth: (parseFloat(prev.bandwidth as unknown as string) + (Math.random() - 0.5) * 0.2).toFixed(1)
      }));
    }, 5000);
